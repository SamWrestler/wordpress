...
if ( is_array( $uploaded_fonts ) AND count( $uploaded_fonts ) > 0 ) {
	foreach ( $uploaded_fonts as $uploaded_font ) {
		...
		if ( count( $urls ) ) {
			...
			$css .= 'font-family:"' . us_sanitize_font_family( $uploaded_font['name'] ) . '";';
...
foreach( $results as $result ) {
	...
	if ( $settings['width'] == 'full' AND ! empty( $settings['stretch'] ) ) {
		$dropdown_css_props .= 'margin: 0 calc( var(--site-content-width) / 2 - 50vw );';
		$dropdown_css_props .= 'padding:' . $settings['padding'] . ' calc( 50vw - var(--site-content-width) / 2 );';
