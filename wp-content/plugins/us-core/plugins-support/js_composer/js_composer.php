...
if ( ! function_exists( 'us_vc_init_shortcodes' ) ) {
	...
	function _us_vc_param( $param_name, $param ) {
		...
		$vc_param = array(
			...
			'dynamic_values' => isset( $param['dynamic_values'] ) ? $param['dynamic_values'] : NULL,
		...
		// For US Link
		if ( $type == 'us_link' AND isset( $param['link_additional_values'] ) ) {
			$vc_param['link_additional_values'] = (array) $param['link_additional_values'];
		}
...
if ( ! function_exists( 'us_vc_field' ) ) {
	...
	function us_vc_field( $vc_field, $value ) {
		...
		// Add additional values for the link (displayed in a popup window)
		if ( $type == 'us_link' AND isset( $vc_field['link_additional_values'] ) ) {
			$field['link_additional_values'] = (array) $vc_field['link_additional_values'];
		}
