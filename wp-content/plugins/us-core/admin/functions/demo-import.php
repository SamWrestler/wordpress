		...
		foreach ( $demos as $name => $import ) { ?>
			<div class="w-importer-item" data-demo-id="<?php echo $name; ?>" data-with-force-options="<?php echo (int) isset( $import[ 'force_theme_options' ] ); ?>">
				...
				<div class="w-importer-message done">
					...
					<p><?php echo sprintf( __( 'Just check the result on %syour site%s or start customize via %sTheme Options%s.', 'us' ), '<a href="' . home_url() . '" target="_blank">', '</a>', '<a href="' . admin_url( 'admin.php?page=us-theme-options' ) . '">', '</a>' ) ?></p>
				</div>
			</div>
		<?php }
		return ob_get_clean();
...
if ( ! function_exists( 'us_upload_demo_import_file' ) ) {
	...
	function us_upload_demo_import_file( $filename = '', $extension = 'xml' ) {
		...
		if (
			...
			AND strlen( $us_api_response ) > 300
			AND $fp = fopen( $file_path, 'w' )
			AND fwrite( $fp, $us_api_response )
		) {
			return $file_path;
		}
