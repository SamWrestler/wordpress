	...
	final class USBuilder {
		...
		public function __construct() {
			...
			if ( is_admin() ) {
				...
				if ( static::is_preview_page() ) {
					...
					add_action( 'wp', array( $shortcode, 'set_page_data' ), 1 );
		...
		public function init_builder_page() {
			...
			if ( is_array( $uploaded_fonts ) and count( $uploaded_fonts ) > 0 ) {
				foreach ( us_get_option( 'uploaded_fonts', array() ) as $font_key => $font_value ) {
					$_font_name = us_sanitize_font_family( $font_value['name'] );
					$custom_fonts_names[ $font_key ] = $_font_name;
				}
			}
			...
			$usb_settings += array(
				...
				'elm_titles' => (array) apply_filters( 'usb_settings_elm_titles', /* value */array() ),
				...
				'designOptions' => array_merge(
					...
					array(
						...
						'customFontsNames' => $custom_fonts_names,
					)
				),
			...
			foreach ( us_config( 'shortcodes.theme_elements', array(), TRUE ) as $elm_filename ) {
				if ( $elm_config = us_config( "elements/$elm_filename", array() ) ) {
					...
					if ( $elm_icon ) {
						$usb_settings['elm_icons'][ $elm_filename ] = $elm_icon;
					}
			...
			foreach ( $fieldsets as $elm_name => $fieldset ) {
				...
				// List of elements supported by the builder
				$usb_settings['elms_supported'][] = $elm_name;
