...
if ( ! function_exists( 'us_api' ) ) {
	 ...
	 * Note: The function is duplicated in `common/functions/functions.php`
	 *
	...
	function us_get_font_family( $font_name ) {
		...
		if ( in_array( $font_name, array( 'body', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6' ) ) ) {
			...
			if ( is_array( $uploaded_fonts ) and count( $uploaded_fonts ) > 0 ) {
				foreach ( $uploaded_fonts as $uploaded_font ) {
					$uploaded_font_name = us_sanitize_font_family( $uploaded_font['name'] );
					if ( $font_name === $uploaded_font_name ) {
						$font_name = '"' . $font_name . '"';
						break;
					}
				}
...
if ( ! function_exists( 'us_generate_link_atts' ) ) {
	 ...
	 */
	function us_generate_link_atts( $link = '', $additional_data = array() ) {
		if ( is_string( $link ) ) {
		...
		if ( $link_type == 'post' OR $link_type == 'popup_post' ) {
			...
			// Use the provided ID to get the image url
			$link_atts['url'] = get_author_posts_url( $user_id, get_the_author_meta( 'user_nicename' ) );

			// TYPE: Author Website
		} elseif ( $link_type == 'author_website' ) {
			$link_atts['url'] = get_the_author_meta( 'url' );

			// TYPE: Custom field
		} elseif ( $link_type == 'custom_field' AND ! empty( $link_atts['custom_field'] ) ) {
			$meta_value = us_get_custom_field( $link_atts['custom_field'], /* acf_format */FALSE );
...
if ( ! function_exists( 'us_get_link_options' ) ) {
	/**
	 * Generate array for "Link" option, used in theme elements
	 *
	 * @param array $additional_types List of options for adding
	 * @return array Returns an array of options for the reference
	 *
	 * Example of available key formats:
	 *		'post' => { type: 'post', ... },
	 *		'custom_field|us_tile_link' => { type: 'custom_field', 'custom_field': 'us_tile_link', ... },
	 */
	function us_get_link_options( $additional_types = array() ) {
		$link_options = array();

		// Additional values
		if ( ! empty( $additional_types ) ) {
			$link_options[ '' ] = (array) $additional_types;
		}

		// Predefined values
		$link_options[ __( 'Custom appearance in Grid', 'us' ) ] = array(
		...
		if ( us_get_option( 'enable_testimonials', 1 ) ) {
			$link_options[ __( 'Testimonial', 'us' ) ][ 'custom_field|us_testimonial_link' ] = __( 'Author Link', 'us' );
		}

		// Expanding the list of options from the filters
		return (array) apply_filters( 'us_get_link_options', $link_options );
	}
}
