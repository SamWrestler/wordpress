...
! function( $, undefined ) {
	 ...
	 */
	var _REGEXP_RELATED_FIELD_NAME_ = /^([A-z\d_\-]+)_value$/;
	...
	$usof.field[ 'link' ] = {
		init: function() {
			...
			self.$dynamicValue = $( '.usof-link-dynamic-value', self.$row );
			...
			self.hasDynamicValues = ! $ush.isUndefined( self.options['dynamicValues'] );
			...
			if ( self.hasDynamicValues ) {
				...
				self.popup = new $usof.popup( self.popupId, {
					...
					init: function() {
						/*popup*/this.$container.off( 'click' ).on( 'click', '[data-dynamic-value]', self._events.selectDynamicValue );
					},
					// Handler is called before the popup show
					beforeShow: function() {
						var dynamicValue = self.$dynamicValue.data( 'value' );

						// Sync of dynamic value with popup buttons
						self._resetPopupButtons(); // reset all buttons in the popup
						if ( ! $ush.isUndefined( dynamicValue ) ) {
			...
			self.$urlSearchResults.off()
				.on( 'mousedown', '.usof-link-search-results-item', self._events.urlSearchResultsSelect.bind( self ) )
				.on( 'scroll', self._events.urlSearchResultsScroll.bind( self ) );
		...
		_selectDynamicValue: function( e ) {
			...
			if ( $target.hasClass( 'active' ) ) {
				...
				$usof.hidePopup( '' + self.popupId ); // hide a popup by its id
			} else {
		...
		setDynamicValue: function( value ) {
			...
			var self = this,
				title = ( self.options.dynamicValues || {} )[ value ] || value;
			self.$inputUrl.addClass( 'hidden' ).val( '' );
			self.$dynamicValue
				.data( 'value', value ) // save value
				.removeClass( 'hidden' )
				.find( '.usof-link-dynamic-value-title' )
				.attr( 'title', title )
				.text( title );
		},
		...
		setValue: function( value, quiet ) {
			...
			if ( ! $.isEmptyObject( data ) ) {
				...
				// Set the url
				if ( ! $ush.isUndefined( data.url ) ) {
					self.$inputUrl.val( data.url );
				}

				// Set the dynaminc value
				else if( data.type ) {
					if ( ! $ush.isUndefined( data[ data.type ] ) ) {
